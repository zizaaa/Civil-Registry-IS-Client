...
import { ErrorInput, errorToast, LoaderDefault, ReCaptcha, serverURL } from "../../hooks/imports";
...
function Login() {
    ...
    const verifyCaptcha = async(value: string | null) => {
        if (value) {
            try {
                await axios.post(`${serverURL}/verify-captcha`,{token:value});
            ...
            console.log("Captcha verification failed or not completed.");
        }
    };
    ...
    return (
        <>
            {showCaptcha ? (
                <ReCaptcha verifyCaptcha={verifyCaptcha} />
            ) : (
                <div className="flex flex-row max-[400px]:w-full items-center max-[400px]:mx-5 mx-10 gap-10 bg-gray-100 drop-shadow-md p-5 rounded-md">
                    <figure className="flex-1 flex flex-col items-center p-5 rounded-e-md max-[800px]:hidden">
                        <img src="/logo.png" className="w-auto h-52" alt="Log in illustration" />
                    ...
                    <form className="flex max-[400px]:w-full w-96 flex-col gap-4 p-5" onSubmit={(e) => login(e)}>
                        ...
                        <div className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-2">
                                <Checkbox
                                    ...
                                    onChange={(e) =>
                                        setCredentials((prev) => ({ ...prev, remember: e.target.checked }))
                                    }
                                />
                        ...
                        <Button type="submit">
                            {mutation.isPending ? <LoaderDefault /> : "Sign in"}
                        </Button>
                    </form>
                </div>
            )}
