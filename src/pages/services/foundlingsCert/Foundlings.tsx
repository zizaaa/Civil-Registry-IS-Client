...
function Foundlings() {    
    ...
    const performSearch = async (term: string) => {
        ...
        try {
            const { data } = await axios.get(`${serverURL}/api/cris/foundling-certificate/search`,{
                params:{
                    searchTerm:term
                },
                withCredentials: true,
    ...
    const handleNavigateSearchedData = (searchedData:SearchType) =>{
        if(searchedData.scannedFile){
            navigate(`/foundlings/preview/file/${searchedData.id}`)
        }else{
            navigate( `/foundlings/preview/${searchedData.id}`)
        }
    }
    ...
    return (
        <section>
            ...
            <div className='flex items-end justify-between mt-5 px-2'>
                {/* Search Input */}
                <div className='flex items-end w-72 relative'>
                            ...
                            (
                                ...
                                (
                                    searchResult.map((data, index)=>(
                                        <div onClick={()=>{handleNavigateSearchedData(data)}} key={index} className='flex items-center gap-5 border-b-[1px] border-slate-300 p-1 cursor-pointer'>
            ...
            <div className='w-full p-2'>
                ...
                <table className='w-full border-collapse rounded-lg shadow-md overflow-hidden'>
                    ...
                    <thead className='bg-gradient-to-r from-darkCyan to-darkBlueTeel text-white'>
                        <tr>
                            <th rowSpan={1} className='px-2 py-3 text-left'>Registry No.</th>
