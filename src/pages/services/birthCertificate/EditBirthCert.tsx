...
function EditBirthCert() {
    ...
    const mutation = useMutation({
        mutationFn: async(formData:BirthCertDataType)=>{
            const { data } = await axios.post(`${serverURL}/api/cris/birth-certificate/update/${id}`,{formData},{ withCredentials:true });
    ...
    const handleUpdate = () =>{
        mutation.mutate(birthCertCredentials);
    }

    useEffect(()=>{
        if (!isLoading && data) {
            for (const [dataKey, dataValue] of Object.entries(data)) {
                setBirthCertCredentials((prev) => ({
                    ...prev,
