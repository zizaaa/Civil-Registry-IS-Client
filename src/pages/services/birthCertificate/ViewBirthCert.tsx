...
function ViewBirthCert() {
    ...
    const handleScreenshotAndSaveAsPDF = async () => {
        ...
        try {
            ...
            activityMutation.mutate(`Downloaded birth certificate for ${data?.one_first} (Registry No. ${data?.registryNumber})`);
        } catch (error) {
        ...
        <div>
            <div className="w-full flex items-end justify-end px-4">
                <Tooltip content="Download">
                    <button onClick={handleScreenshotAndSaveAsPDF} className='p-2.5 ms-2 text-sm font-medium text-white bg-darkCyan rounded-md drop-shadow-md border border-darkCyan hover:bg-darkBlueTeel'>
                        <MdFileDownload />
